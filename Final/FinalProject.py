...
def inputPlayerLetter():
    ...
    # Returns a list with the player's letter as the first item, and the player2's letter as the second.
    letter = ''
    while not (letter == 'X' or letter == 'O'):
        print('Player 1, Do you want to be X or O?')
        letter = input().upper()

    # the first element in the tuple is the player's letter, the second is the player2's letter.
    if letter == 'X':
        return ['X', 'O']
    else:
        return ['O', 'X']
...
def getplayer2Move(board, player2Letter):
    # Given a board and the player2's letter, determine where to move and return that move.
    if player2Letter == 'X':
        playerLetter = 'O'
    else:
        playerLetter = 'X'
